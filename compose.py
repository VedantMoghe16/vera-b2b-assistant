...
def _resolve_references(category: dict, merchant: dict, trigger: dict,
    ...
    if top_item_id:
        ...
        if not match:
            # Don't return None — just skip the digest item
            pass
        else:
...
KIND_PRIORS = {
    ...
    "renewal_due":              1.6,
    # Map judge "type" values directly too
    "research_spike":           1.2,
    "perf_spike":               0.9,
    "recall":                   1.6,
    "lapse":                    1.3,
...
def _specificity_score(trigger: dict, merchant: dict) -> float:
    ...
    # Also count signal field at top level
...
def _has_yes_no_pivot(trigger: dict) -> bool:
    yes_no_kinds = {
        ...
        "regulation_change", "competitor_opened", "research_spike",
        "recall", "lapse",
    }
...
def _build_rationale(winner: dict, candidates: list[dict],
    ...
    templates = {
        "research_digest":          "External research item with merchant-relevant clinical anchor.",
        "research_spike":           "Local search spike detected — high intent window open now.",
        "supply_alert":             "Patient-safety hard override — supply alert beats all candidates.",
        "active_planning_intent":   "Merchant-initiated planning thread; continuing open intent.",
        "perf_dip":                 "Severe perf dip with concrete, actionable fix.",
        "recall_due":               "Customer recall window opened with consent + saved preferences.",
        "chronic_refill_due":       "Chronic Rx subscription due; customer consented.",
        "ipl_match_today":          "Match-day operator advice with contrarian judgment.",
        "review_theme_emerged":     "Operational theme rising; flagging early.",
        "gbp_unverified":           "Highest-leverage low-effort lift. Single yes/no.",
        "wedding_package_followup": "Bridal skin-prep window opens 30 days pre-wedding.",
        "customer_lapsed_hard":     "57+ days lapsed; warm reactivation.",
        "regulation_change":        "Compliance deadline approaching.",
        "recall":                   "Customer recall window open — re-engagement moment.",
        "lapse":                    "Customer lapsed — winback window active.",
    }

    base = templates.get(kind, f"Selected {kind} as highest-scoring from {n_candidates} candidates.")
    return base
